...
namespace DuiLib
{
	...
	public:
		...
		void OnErrPageReload();
		bool OnJsMessage(DWORD dwMsg);
		void Navigate2(LPCTSTR lpszUrl, HWND hWnd=NULL);
		void Refresh();
		...
		void SetHomePage(CDuiString szHomePage,bool bAutoNavi);
		HWND GetInternetExplorerServerWnd();
		bool ExecJsFun(const wstring& lpJsFun, const vector<wstring>& params);
	protected:
		HWND m_hParentWnd;//发送消息窗口
		IWebBrowser2*	m_pWebBrowser2; //浏览器指针
		IHTMLWindow2*	m_pHtmlWnd2;
		CDuiString m_szErrPageUrl;//错误指向地址
		LONG m_dwRef;
		DWORD m_dwCookie;
		...
		void DocComplete(IDispatch *pDisp, VARIANT* sURL);
