...
void ClientHandler::CreateBrowser(HWND hWnd, RECT &rc, const std::wstring& url)
{
	CefWindowInfo info;
	...
	info.SetAsChild(hWnd, rc);
	//info.SetAsWindowless(hParentWnd, false);
	CefBrowserHost::CreateBrowserSync(info, this, url, settings, NULL, NULL);
}
...
bool ClientHandler::OnConsoleMessage(CefRefPtr<CefBrowser> browser,
  ...
  FILE* file = NULL;
  fopen_s(&file, console_log_file_.c_str(), "a");
  if (file) {
    std::stringstream ss;
    ss << "Level: ";
    switch (level) {
...
void ClientHandler::OnDownloadUpdated(
  ...
  if (download_item->IsComplete()) {
    Alert(browser, "File \"" + download_item->GetFullPath().ToString() + "\" downloaded successfully.");
  }
}
    ...
    bool* no_javascript_access) 
{
	CEF_REQUIRE_UI_THREAD();
 	if (delegate_)
 		return delegate_->OnOpenNewUrl(target_url);
...
void ClientHandler::OnLoadStart(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, TransitionType transition_type)
{
	CEF_REQUIRE_UI_THREAD();
	if (delegate_)
		delegate_->OnLoadStart(browser, frame, transition_type);
}
                                   ...
                                   bool is_redirect) 
{
	...
	if (delegate_)
		return delegate_->OnBeforeBrowse(browser, url);
	return false;
}
...
//  		  windowInfo, client, settings);
	  if (delegate_)
		  has_devtools = delegate_->OnShowDevTools(browser, windowInfo, client, settings);
  }
