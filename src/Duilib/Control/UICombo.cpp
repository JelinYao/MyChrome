...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
		...
		for (DWORD i = 0; i < pParentPM->GetCustomFontCount(); i ++)
		{//kk_修改 继承父类的字体信息
			TFontInfo* fontinfo = pParentPM->GetFontInfo(i);
			if(fontinfo)
				m_pm.AddFont(fontinfo->szFaceName,fontinfo->iSize,fontinfo->bBold,fontinfo->bUnderline,fontinfo->bItalic);
		}
     ...
     //   EnsureVisible(m_pOwner->GetCurSel()); 注释 不注释的化 滚动有选中时只能滚动到选中位置
        return 0;
    }
	...
	{//修改 窗口第一次绘制的时候 调整滚动条位置 到当前选中项 增加变量bIsSetPos控制 该代码只执行一次
		if (uMsg == WM_PAINT && bIsSetPos)
		{
			EnsureVisible(m_pOwner->GetCurSel());
			bIsSetPos = false;
		}
...
int CComboUI::GetItemIndex(CDuiString szText)
{
	for( int it = 0; it < m_items.GetSize(); it++ ) {
		CControlUI* p = static_cast<CControlUI*>(m_items[it]);
		if (_tcscmp(p->GetText(),szText) == 0)
		{
			return it;
		}
	}
...
bool CComboUI::Add(CDuiString szText,CListLabelElementUI* pListLabel)
{
	if (!pListLabel)
	{
		pListLabel = new CListLabelElementUI;
	}
	pListLabel->SetText(szText);
	return Add(pListLabel);
}
...
bool CComboUI::AddAt(CDuiString szText, int iIndex,CListLabelElementUI* pListLabel)
{
	if (!pListLabel)
	{
		pListLabel = new CListLabelElementUI;
	...
	pListLabel->SetText(szText);
	return AddAt(pListLabel,iIndex);
}
...
bool CComboUI::Remove(CDuiString szText)
{
	int iIndex = GetItemIndex(szText);
	if (iIndex == -1) return false;
...
bool CComboUI::RemoveAt(CDuiString szText)
{
	int iIndex = GetItemIndex(szText);
	if (iIndex == -1) return false;
...
void CComboUI::SetAltBkColor(DWORD dwAltBkColor)
{
	m_ListInfo.dwAltBkColor = dwAltBkColor;
}

DWORD CComboUI::GetAltBkColor() const
{
	return m_ListInfo.dwAltBkColor;
}
...
void CComboUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if ( _tcscmp(pstrName, _T("itemaltbkcolor")) == 0 ) 
	{
		if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
		SetAltBkColor(clrColor);
	}
...
HWND CComboUI::GetCobWnd()
{
	return m_CobWnd;
}

void CComboUI::SetCobWnd(HWND hWnd)
{
	m_CobWnd = hWnd;
}
