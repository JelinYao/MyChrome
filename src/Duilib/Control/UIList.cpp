    ...
    case UIEVENT_SCROLLWHEEL:
        {
            ...
            case SB_LINEUP:
                ...
                else LineUp(LOWORD(event.lParam)/4);
                return;
            case SB_LINEDOWN:
                if( m_bScrollSelect ) SelectItem(FindSelectable(m_iCurSel + 1, true), true);
                else LineDown(LOWORD(event.lParam)/4);
                return;
            }
        }
        break;
...
void CListUI::SetAltBkColor(DWORD dwAltBkColor)
{
...
DWORD CListUI::GetAltBkColor() const
{
...
void CListUI::LineUp(int nPos)
{
	if(nPos < 8) nPos = 8;
    m_pList->LineUp(nPos);
}

void CListUI::LineDown(int nPos)
{
	if(nPos < 8) nPos = 8;
    m_pList->LineDown(nPos);
}
...
void CListElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
{
	...
	if ( pInfo->bAlternateBk && m_iIndex % 2 == 1 ) iAltBkColor = pInfo->dwAltBkColor;
	...
	if (iAltBkColor != 0)  {
		CRenderEngine::DrawColor(hDC, m_rcItem, GetAdjustColor(iAltBkColor));
	}
...
void CListContainerElementUI::DoEvent(TEventUI& event)
{
	...
	case UIEVENT_BUTTONDOWN:
		//m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMCLICK);
		m_pManager->SendNotify(this, DUI_MSGTYPE_LBTN_ITEMCLICK);
		Select();
		Invalidate();
		break;
	case UIEVENT_RBUTTONDOWN:
		Select();
		Invalidate();
		m_pManager->SendNotify(this, DUI_MSGTYPE_RBTN_ITEMCLICK);
		break;
...
void CListContainerElementUI::SetCustom(bool bCustom)
{
	m_bCustom = bCustom;
}
...
IListCallbackUI* CListContainerElementUI::GetTextCallback() const
{
	return m_pCallback;
}

void CListContainerElementUI::SetTextCallback(IListCallbackUI* pCallback)
{
	m_pCallback = pCallback;
}
...
void CListContainerElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
{
	...
	if(m_bCustom){
		...
		if(pCallback){
			...
			for (int i = 0; i < m_items.GetSize();i++)
			{
				...
				if(pControl->GetTag() == 3){//检查背景 
					szText = pControl->GetBkImage();
					szText = pCallback->GetItemText(pControl,m_iIndex,i);
					if(!szText.IsEmpty())
						pControl->SetBkImage(szText);
				}else if(pControl->GetTag() == 4){//检测文字
					szText = pCallback->GetItemText(pControl,m_iIndex,i);
					if(!szText.IsEmpty())
    ...
    if( IsSelected() ) {
		if(pInfo->dwSelectedBkColor != 0xFFFFFFFF && pInfo->dwSelectedBkColor != 0)
			iBackColor = pInfo->dwSelectedBkColor;
    }
    if( !IsEnabled() ) {
        iBackColor = pInfo->dwDisabledBkColor;
