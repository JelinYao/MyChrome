...
HRESULT CRichEditUI::TxSendMessage(UINT msg, WPARAM wparam, LPARAM lparam, LRESULT *plresult) const
{
    if( m_pTwh ) {
        if( msg == WM_KEYDOWN && TCHAR(wparam) == VK_RETURN ) {
			if ( ( ::GetKeyState(VK_CONTROL)>0 && !m_bWantReturn ) || (::GetKeyState(VK_CONTROL)<0 && !m_bWantCtrlReturn ) )
			{
            ...
            case WM_RBUTTONUP:
                {
					...
					{
						GetManager()->SendNotify(this, DUI_MSGTYPE_EDIT_RBTNUP);
					}
                }
                break;
            }
