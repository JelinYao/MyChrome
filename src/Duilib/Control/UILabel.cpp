...
namespace DuiLib
{
	...
	GdiplusStartupInput CLabelUI::m_gdiplusStartupInput;
	ULONG_PTR CLabelUI::m_gdiplusToken;
	UINT CLabelUI::m_nCount	= 0;

	CLabelUI::CLabelUI() : m_uTextStyle(DT_LEFT | DT_VCENTER | DT_SINGLELINE), m_dwTextColor(0), 
	...
	void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if (_tcscmp(pstrName,_T("multiline")) == 0)
		{
			...
			if( _tcsstr(pstrValue, _T("left+top")) != NULL ) {
				m_uTextStyle = (DT_LEFT | DT_TOP | m_dwMultiline);
			}
			if( _tcsstr(pstrValue, _T("left+vcenter")) != NULL ) {
				m_uTextStyle = (DT_LEFT | DT_VCENTER | DT_SINGLELINE);
			}
			if( _tcsstr(pstrValue, _T("left+bottom")) != NULL ) {
				m_uTextStyle = (DT_LEFT | DT_BOTTOM | m_dwMultiline);
			}
			if( _tcsstr(pstrValue, _T("right+top")) != NULL ) {
				m_uTextStyle = (DT_RIGHT | DT_TOP | m_dwMultiline);
			}
			if( _tcsstr(pstrValue, _T("right+vcenter")) != NULL ) {	
				m_uTextStyle = (DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
			}
			if( _tcsstr(pstrValue, _T("right+bottom")) != NULL ) {
				m_uTextStyle |= (DT_RIGHT | DT_BOTTOM | m_dwMultiline);
			}
			if( _tcsstr(pstrValue, _T("center+top")) != NULL ) {
				m_uTextStyle = (DT_CENTER | DT_TOP | DT_SINGLELINE);
			}
			if( _tcsstr(pstrValue, _T("center+vcenter")) != NULL ) {	
		...
		else if (_tcscmp(pstrName, _T("effectalign")) == 0)
		{
		...
		else if (_tcscmp(pstrName, _T("effectvalign")) == 0)
		{
	...
	void CLabelUI::PaintText(HDC hDC)
	{
		...
		if (pManager)
		{
			 m_TextValue = pManager->GetLanguageAttributeList(CControlUI::GetText());
	...
	UINT CLabelUI::GetEffectAlign()
	{
		return m_hAlign;
	}

	UINT CLabelUI::GetEffectVAlign()
	{
		return m_vAlign;
	}
	//************************************
	...
	void CLabelUI::SetShadowOffset( int _offset,int _angle )
	{
		try
		{
			...
			m_szOffset.cx = _offset;
			m_szOffset.cy = _angle;
		}
		catch (...)
		{
	...
	SIZE CLabelUI::GetShadowOffset1()
	{
		return m_szOffset;
	}
	...
	UINT CLabelUI::GetMultiLineStyle()
	{
		return m_dwMultiline;
	}
	void CLabelUI::SetMultiLine(bool bMultiLine)
	{
		...
		if (m_bmultiline)
			m_dwMultiline = DT_WORDBREAK | DT_EDITCONTROL;
		else	
			m_dwMultiline = DT_SINGLELINE;
		Invalidate();
	}
