	...
	void CScrollBarUI::PaintBk(HDC hDC)
	{
		...
		if( (m_uThumbState & UISTATE_DISABLED) != 0 ) {
			if( !m_sBkDisabledImage.IsEmpty() ) {
				RECT rcItem;
				rcItem.top = m_rcItem.top + DEFAULT_SCROLLBAR_SIZE;
				rcItem.left = m_rcItem.left;
				rcItem.right = m_rcItem.right;
				rcItem.bottom = m_rcItem.bottom - DEFAULT_SCROLLBAR_SIZE;
				RECT rcPaint;
				rcPaint.left = m_rcPaint.top + DEFAULT_SCROLLBAR_SIZE;
				rcPaint.left = m_rcPaint.left;
				rcPaint.right = m_rcPaint.right;
				rcPaint.bottom = m_rcPaint.bottom - DEFAULT_SCROLLBAR_SIZE;
				if(!CRenderEngine::DrawImageString(hDC, m_pManager, rcItem, m_rcPaint, m_sBkDisabledImage, NULL)) m_sBkDisabledImage.Empty();
				else return;
			}
		}
			...
			if( !m_sBkPushedImage.IsEmpty() ) {
				...
				rcPaint.right = m_rcPaint.right;
				rcPaint.bottom = m_rcPaint.bottom - DEFAULT_SCROLLBAR_SIZE;
				if(!CRenderEngine::DrawImageString(hDC, m_pManager, rcItem, m_rcPaint, m_sBkPushedImage, NULL)) m_sBkPushedImage.Empty();
				else return;
			}
		}
			...
			if( !m_sBkHotImage.IsEmpty() ) {
				...
				rcPaint.right = m_rcPaint.right;
				rcPaint.bottom = m_rcPaint.bottom - DEFAULT_SCROLLBAR_SIZE;
				if(!CRenderEngine::DrawImageString(hDC, m_pManager, rcItem, m_rcPaint, m_sBkHotImage, NULL)) m_sBkHotImage.Empty();
				else return;
			}
		}
		...
		if( !m_sBkNormalImage.IsEmpty() ) {
			...
			rcPaint.right = m_rcPaint.right;
			rcPaint.bottom = m_rcPaint.bottom - DEFAULT_SCROLLBAR_SIZE;
			if(!CRenderEngine::DrawImageString(hDC, m_pManager, rcItem, m_rcPaint, m_sBkNormalImage, NULL)) m_sBkNormalImage.Empty();
			else return;
		}
	}
