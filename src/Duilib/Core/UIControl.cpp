...
CControlUI::~CControlUI()
{
	...
	if (m_pGifHandler)
	{
		m_pManager->KillTimer(static_cast<CControlUI*>(this),GIF_TIMER_ID);
...
void CControlUI::SetGifed(bool bPlayGifed)
{
	m_bPlayGif = bPlayGifed;
	Invalidate();
}
...
void CControlUI::SetVisible(bool bVisible)
{
	...
	if(bVisible == false && m_pGifHandler)
		m_pManager->KillTimer(this, GIF_TIMER_ID);
...
// 		POINT pt={0,0};
// 		::GetCursorPos(&pt);
// 		::ScreenToClient(m_pManager->GetTooltipWindow(),&pt);
// 		PostMessage(m_pManager->GetTooltipWindow(),WM_MOUSEMOVE,NULL,MAKELPARAM(pt.x,pt.y));
        return;
    }
    ...
    {
		if (m_pGifHandler && event.wParam == GIF_TIMER_ID)
		{
			m_bPlayGif = true;
			NeedUpdate();
		}
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("default")) == 0 ) ApplyAttributeList(GetManager()->GetDefaultAttributeList(pstrValue));
}
...
//	SaveControlImage(hDC);
}

bool CControlUI::IsCanDraw()
{
	return m_bCanDraw;
}

void CControlUI::SetCanDraw(bool bCanDraw)
{
	if(m_bCanDraw == bCanDraw) return;
	m_bCanDraw = bCanDraw;
}
...
void CControlUI::PaintBkImage(HDC hDC)
{
	...
	CDuiString szBkImagePos = m_sBkImage.Right(3); 
	if (szBkImagePos.CompareNoCase(_T("gif")) == 0)
	{
		TImageInfo* pImageInfo = NULL;
		if (m_pGifHandler == NULL)
		{
			m_pGifHandler = CRenderEngine::LoadGif(m_sBkImage.GetData(),0, 0);
			//m_pManager->SetTimer(static_cast<CControlUI*>(this),GIF_TIMER_ID,m_pGifHandler->GetCurrentFrameInfo()->delay);
		}
		if (m_pGifHandler == NULL)
		{
		...
		{
			m_bPlayGif = false;
			pImageInfo = m_pGifHandler->GetNextFrameInfo();
		}
		else
		{
			pImageInfo = m_pGifHandler->GetCurrentFrameInfo();
		}
		if (pImageInfo)
		{
			RECT rcBmpPart = {0};
	...
	{
		if (m_bGetRegion && IsRandom())
		{
			HRGN hRgn = GetRegion(m_sBkImage,RGB(0,0,0),0,m_rcPaint.left,m_rcPaint.top);
			if (hRgn)
			{
				m_hRgn = hRgn;
			}
			if(m_hRgn == NULL)
				m_hRgn = CreateRectRgn(m_rcPaint.left,m_rcPaint.top,m_rcPaint.right,m_rcPaint.bottom); //定义新的空的HRGN.不能初始化为NULL
		}

		if ( !DrawImage(hDC, (LPCTSTR)m_sBkImage) ) m_sBkImage.Empty();
		if ( !m_sForeImage.IsEmpty() )
		{
			const TImageInfo* data=m_pManager->GetImageEx((LPCTSTR)m_sForeImage, NULL, 0);
			if ( data )
			{//新增状态叠加alpha效果背景
				HDC		hMemDC=CreateCompatibleDC(hDC);
				HBITMAP hOldBmp=(HBITMAP)SelectObject(hMemDC, data->hBitmap);
				BLENDFUNCTION bf={ AC_SRC_OVER, 0, m_bForeAlpha, AC_SRC_ALPHA };
				::AlphaBlend(hDC, m_rcItem.left, m_rcItem.top, m_rcItem.right-m_rcItem.left, \
					m_rcItem.bottom-m_rcItem.top, hMemDC, 0, 0, data->nX, data->nY, bf);
...
HRGN CControlUI::GetRegion(HBITMAP hBmp,COLORREF cTransparentColor,COLORREF cTolerance,int xoffset,int yoffset)
{
	HRGN hRgn=NULL;
	if(hBmp)
	{
...
HRGN CControlUI::GetRegion(CDuiString pStrImage,COLORREF cTransparentColor,COLORREF cTolerance,int xoffset,int yoffset)
{
	HRGN hRgn=NULL;
	...
	HBITMAP hBmp = data->hBitmap;
	
// 	CDuiImage* pImg = new CDuiImage;
// 	pImg->Save(L"D:\\11.png",Gdiplus::ImageFormatPNG);
// 	HBITMAP hBmp = pImg->Detach();
	if(hBmp)
	{
		//Create a memory DC inside which we will scan the bitmap content
		HDC hMemDC=CreateCompatibleDC(NULL);
...
void CControlUI::SetUserValue( UINT_PTR pValue )
{
	m_pValue=pValue;
}

UINT_PTR CControlUI::GetUserValue() const
{
	return m_pValue;
}
...
void CControlUI::SetForeAlpha( const BYTE bAlpha )
{	
	m_bForeAlpha=bAlpha;
	Invalidate();
}

void CControlUI::SetImage( LPCTSTR lpBkImage, LPCTSTR lpForeImage, BYTE bAlpha )
{
	m_sBkImage	=lpBkImage;
	m_sForeImage=lpForeImage;
	m_bForeAlpha=bAlpha;
	Invalidate();
}
