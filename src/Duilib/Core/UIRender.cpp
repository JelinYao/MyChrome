...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPCTSTR type, DWORD mask)
{
	...
	do 
	{
		if( type == NULL ) 
		{
			if( _tcslen(CPaintManagerUI::GetResourceZip()) == 0 ) 
			{
    ...
    TImageInfo* data = LoadImage(pData, dwSize, mask);
	delete[] pData;
    return data;
}
...
{
	CDuiString szText = pstrText;
	if (pManager && (szText.Find(_T("ID")) != -1))
	{
		szText = pManager->GetLanguageAttributeList(pstrText);
		if (szText.IsEmpty())
		{
			szText = pstrText;
		}
    ...
    ::DrawText(hDC, szText.GetData(), -1, &rc, uStyle | DT_NOPREFIX);
...
int CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, CDuiString* sLinks, int& nLinkRects, UINT uStyle)
{
    ...
    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
    if( pstrText == NULL || pManager == NULL ) return 0;
            ...
            case _T('a'):  // Link
                {
                    ...
                    if( pFontInfo->bUnderline == false ) {
                        HFONT hFont = pManager->GetFont(pFontInfo->szFaceName, pFontInfo->iSize, pFontInfo->bBold, true, pFontInfo->bItalic);
                        if( hFont == NULL ) hFont = pManager->AddFont(pFontInfo->szFaceName, pFontInfo->iSize, pFontInfo->bBold, true, pFontInfo->bItalic);
            ...
            case _T('b'):  // Bold
                {
                    ...
                    if( pFontInfo->bBold == false ) {
                        HFONT hFont = pManager->GetFont(pFontInfo->szFaceName, pFontInfo->iSize, true, pFontInfo->bUnderline, pFontInfo->bItalic);
                        if( hFont == NULL ) hFont = pManager->AddFont(pFontInfo->szFaceName, pFontInfo->iSize, true, pFontInfo->bUnderline, pFontInfo->bItalic);
                        pFontInfo = pManager->GetFontInfo(hFont);
                        aFontArray.Add(pFontInfo);
                        pTm = &pFontInfo->tm;
                        ::SelectObject(hDC, pFontInfo->hFont);
                ...
                {    
                    ...
                    if( sName.IsEmpty() ) { // Italic
                        ...
                        if( pFontInfo->bItalic == false ) {
                            HFONT hFont = pManager->GetFont(pFontInfo->szFaceName, pFontInfo->iSize, pFontInfo->bBold, pFontInfo->bUnderline, true);
                            if( hFont == NULL ) hFont = pManager->AddFont(pFontInfo->szFaceName, pFontInfo->iSize, pFontInfo->bBold, pFontInfo->bUnderline, true);
                            pFontInfo = pManager->GetFontInfo(hFont);
                            aFontArray.Add(pFontInfo);
                            pTm = &pFontInfo->tm;
                            ::SelectObject(hDC, pFontInfo->hFont);
