...
CControlUI* CDialogBuilder::Create(CMarkup* xml,IDialogBuilderCallback* pCallback, CPaintManagerUI* pManager, CControlUI* pParent)
{
	ASSERT(xml);
    m_pCallback = pCallback;
    ...
    if( pManager ) {
        ...
        for( CMarkupNode node = root.GetChild() ; node.IsValid(); node = node.GetSibling() ) {
			...
			else if ( _tcscmp(pstrClass, _T("Language")) == 0 )
			{
				if( !node.HasAttributes() ) continue;
				if (pManager->bLoadingLanguage) continue; //如果已经加载过了 不再加载
				int count = 1;
				LPTSTR pstr = NULL;
				TCHAR szValue[500] = { 0 };
				SIZE_T cchLen = lengthof(szValue) - 1;
				if ( !node.GetAttributeValue(_T("xmlfile"), szValue, cchLen) ) continue ;
				...
				if( m_pstrtype != NULL ) { // 使用资源dll，从资源中读取
					...
					builder.Create((LPCTSTR)szValue, (UINT)0, m_pCallback, pManager, pParent);
				}	
				pManager->bLoadingLanguage = true;
			}
			else if (_tcscmp(pstrClass, _T("lang")) == 0)
			{
				nAttributes = node.GetAttributeCount();
				LPCTSTR pControlName = NULL;
				LPCTSTR pControlValue = NULL;
				...
				if( pControlName ) {
					pManager->AddLanguageAttributeList(pControlName, pControlValue);
				}
			}
        }
            ...
            if( pManager->GetPaintWindow() ) {
                ...
                for( int i = 0; i < nAttributes; i++ ) {
                    ...
                    if( _tcscmp(pstrName, _T("size")) == 0 ) {
                        ...
                        LPTSTR pstr = NULL;
                        int nx = _tcstol(pstrValue, &pstr, 10);  ASSERT(pstr);    
                        int ny = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr); 
                        pManager->SetRoundCorner(nx,ny);
                    } 
					...
					else if ( _tcscmp(pstrName, _T("escclosewnd")) == 0 )
					{
						pManager->SetEscCloseWnd(_tcscmp(pstrValue, _T("true")) == 0);
					}
					else if ( _tcscmp(pstrName, _T("aero")) == 0 )
					{
						pManager->SetAero(_tcscmp(pstrValue, _T("true")) == 0);
					}
					else if ( _tcscmp(pstrName, _T("margins")) == 0 )
					{
						...
						int cx = _tcstol(pstrValue, &pstr, 10);  ASSERT(pstr);    
						int cy = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr); 
						int nx = _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);    
						int ny = _tcstol(pstr + 1, &pstr, 10); ASSERT(pstr); 
						pManager->SetAeroMargins(cx, cy,nx,ny);
					}
					else if ( _tcscmp(pstrName, _T("shadow")) == 0 )
					{
						pManager->SetShadow(_tcscmp(pstrValue, _T("true")) == 0);
					}
					else if ( _tcscmp(pstrName, _T("shadowborder")) == 0 )
					{
						LPTSTR pstr = NULL;
						int cx = _tcstol(pstrValue, &pstr, 10);  ASSERT(pstr);    
						int cy = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr); 
						int nx = _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);    
						int ny = _tcstol(pstr + 1, &pstr, 10); ASSERT(pstr); 
						pManager->SetShadowBorder(cx,cy,nx,ny);
					}
...
CControlUI* CDialogBuilder::_Parse(CMarkupNode* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
{
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
        ...
        if( _tcscmp(pstrClass, _T("Image")) == 0 || _tcscmp(pstrClass, _T("Font")) == 0 \
            || _tcscmp(pstrClass, _T("Default")) == 0 || _tcscmp(pstrClass,_T("lang")) == 0) continue;
        CControlUI* pControl = NULL;
        if( _tcscmp(pstrClass, _T("ChildLayout")) == 0 ) {
            if( !node.HasAttributes() ) continue;
            int count = 1;
            LPTSTR pstr = NULL;
            TCHAR szValue[500] = { 0 };
		...
		//树控件XML解析
            ...
            case 8:
				...
				else if( _tcscmp(pstrClass, DUI_CTR_OPTIONEX) == 0 )		  pControl = new COptioneExUI;
                break;
            case 9:
            ...
            case 10:
				...
				else if( _tcscmp(pstrClass, DUI_CTR_MENUBUTTON) == 0 )		  pControl = new CMenuButtonUI;
                break;
			case 11:
				if (_tcscmp(pstrClass, DUI_CTR_CHILDLAYOUT) == 0)			  pControl = new CChildLayoutUI;
				break;
			case 12:
				if ( _tcscmp(pstrClass, DUI_CTR_OPTIONLAYOUT) == 0 )		  pControl = new COptionLayoutUI;
				break;
            case 14:
            ...
            case 16:
                ...
                break;
			case 17:
				if( _tcscmp(pstrClass, DUI_CTR_COMBINATIONLAYOUT) == 0 )       pControl = new CCombinationLayoutUI;
				if( _tcscmp(pstrClass, DUI_CTR_TRANSPARENTLAYOUT) == 0 )	   pControl = new CTransparentLayoutUI;
				break;
            case 20:
