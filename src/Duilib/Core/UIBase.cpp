...
void CWindowWnd::CenterWindow()
{
	...
	::MoveWindow(m_hWnd,xLeft,yTop,DlgWidth,DlgHeight,FALSE);//换成movewindow 不然 xp下桌面窗口会闪动 kk
   // ::SetWindowPos(m_hWnd, NULL, xLeft, yTop, -1, -1, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE | SWP_HIDEWINDOW);
}
...
bool CWindowWnd::RegisterWindowClass()
{
    ...
    wc.hbrBackground = GetStockBrush(COLOR_BACKGROUND);
...
BOOL CWindowWnd::ModifyStyleEx(DWORD dwRemove, DWORD dwAdd, UINT nFlags)
{
	ASSERT(::IsWindow(m_hWnd));
...
BOOL CWindowWnd::MoveWindow(int X,int Y,int nWidth,int nHeight,BOOL bRepaint)
{
	ASSERT(::IsWindow(m_hWnd));
	return ::MoveWindow(m_hWnd,X,Y,nWidth,nHeight,bRepaint);
}

BOOL CWindowWnd::MoveWindow(RECT rc,BOOL bRepaint)
{
	ASSERT(::IsWindow(m_hWnd));
	POINT pt;
	pt.x = rc.left;
	...
	return ::MoveWindow(m_hWnd,pt.x,pt.y,rc.right-rc.left,rc.bottom-rc.top,bRepaint);
}

BOOL CWindowWnd::ModifyStyle(DWORD dwRemove, DWORD dwAdd, UINT nFlags)
{
	ASSERT(::IsWindow(m_hWnd));
